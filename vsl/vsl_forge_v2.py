...
def did_generate_avatar(audio_path: Path, out_path: Path) -> bool:
    ...
    try:
        ...
        # 1. Upload do áudio MP3 para o D-ID
        audio_bytes = audio_path.read_bytes()
        boundary = f"----DIDFormBoundary{int(time.time())}"
        ...
        if resp_upload.status_code not in (200, 201):
            log(f"  ⚠️  D-ID upload falhou: {resp_upload.status_code} — {resp_upload.text[:200]}")
            return False
        ...
        talk_id = resp_talk.json().get("id", "")
        if not talk_id:
            log("  ⚠️  D-ID: talk_id não retornado")
            return False

        log(f"  ⏳ D-ID: processando [{talk_id}]…")

        # 3. Polling até ficar pronto (máx 3 minutos)
        for attempt in range(36):
            time.sleep(5)
            resp_status = requests.get(
                f"https://api.d-id.com/talks/{talk_id}",
            ...
            if status == "done":
                result_url = data.get("result_url", "")
                if not result_url:
                    log("  ❌ D-ID: result_url não encontrado")
                    return False
                ...
                if file_ok(out_path, min_bytes=50_000):
                    ...
                    log(f"  ✅ D-ID: avatar gerado ({size_kb}KB)")
                    return True
                else:
                    log("  ❌ D-ID: vídeo baixado mas muito pequeno")
                    return False

            elif status in ("error", "failed"):
                err = data.get("error", {})
                log(f"  ❌ D-ID: erro — {err.get('description', 'desconhecido')}")
                return False
            else:
                if attempt % 3 == 0:
                    log(f"  ⏳ D-ID: {status} ({(attempt+1)*5}s)…")

        log("  ⚠️  D-ID: timeout após 3 minutos")
...
def main() -> int:
    ...
    for idx, scene in enumerate(project.scenes, 1):
        ...
        # Gerar imagem
        img_ok = False
        if not args.draft and hf_ok:  # imagem apenas no modo full
        ...
        if use_did and file_ok(audio_path):  # avatar sempre, draft ou não
        ...
        if did_ok and file_ok(avatar_path, min_bytes=50_000):
            ...
            # Overlay limpo: avatar D-ID no canto inferior direito
            # Áudio vem da cena principal (ElevenLabs)
            run([
                "ffmpeg", "-y",
                "-i", str(scene_path),
                "-i", str(avatar_path),
                "-filter_complex",
                (
                    f"[1:v]scale={avatar_w}:{avatar_h}:flags=lanczos,"
                    f"format=yuv420p[avatar];"
                    f"[0:v][avatar]overlay=x={x_pos}:y={y_pos}:shortest=1[vout]"
                ),
                "-map", "[vout]",
                "-map", "0:a",
