...
export default function AdminDashboard() {
  ...
  const { data: stats, isLoading: loadingStats } = trpc.admin.stats.useQuery(undefined, { retry: false });
  const { data: users  = [] } = trpc.admin.users.useQuery(undefined, { retry: false });
  const { data: projects = [] } = trpc.admin.projects.useQuery(undefined, { retry: false });
  const { data: overview, isLoading: loadingOv } = (trpc as any).admin?.financialOverview?.useQuery?.(undefined, { retry: false }) ?? { data: null, isLoading: false };
  ...
  const recentTx:  any[] = Array.isArray(overview?.recentTx)  ? overview.recentTx  : [];
  ...
  const totalPlanUsers = Math.max(1, Object.values(planBreakdown).reduce((a: any, b: any) => a + b, 0));
  const PLANS = [
    ...
    <Layout>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: "white", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--black)", margin: "0 0 16px" }}>
            Distribuição de planos
          </h2>
          {PLANS.map(p => {
        ...
        <div style={{ background: "white", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
          ...
          {loadingOv ? (
            <p style={{ fontSize: 13, color: "var(--muted)", textAlign: "center", padding: "20px 0" }}>Carregando...</p>
          ) : topUsers.length === 0 ? (
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < 4 ? "1px solid var(--border)" : "none" }}>
              ...
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "var(--green-dk)" }}>
                  R$ {Number(u.balance || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
            ...
            {(users as any[]).slice(0, 6).map((u: any) => (
              <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 20px" }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 99,
                  background: u.plan === "free" ? "#f1f5f9" : u.plan === "premium" ? "#f0fdf4" : u.plan === "vip" ? "#fdf4ff" : "#eff6ff",
                  color:      u.plan === "free" ? "#6b7280" : u.plan === "premium" ? "#16a34a" : u.plan === "vip" ? "#7c3aed" : "#2563eb" }}>
                  {u.plan?.toUpperCase()}
                </span>
              </div>
            ))}
        ...
        <div style={{ background: "white", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: "8px 0" }}>
            {loadingOv ? (
            ...
            ) : recentTx.slice(0, 6).map((tx: any, i: number) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 20px" }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 700, color: tx.type === "deposit" ? "var(--green-dk)" : "#f59e0b", flexShrink: 0 }}>
                  {tx.type === "deposit" ? "+" : "-"}R$ {Number(tx.amount || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "white", borderRadius: 18, padding: 28, maxWidth: 400, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,.2)" }}>
            ...
            <div style={{ display: "grid", gap: 10, marginBottom: 20 }}>
              {(["free","basic","premium","vip"] as const).map(v => (
                <button key={v} onClick={() => setMyPlanValue(v)}
                  style={{ padding: "12px 16px", borderRadius: 10, border: `2px solid ${myPlanValue === v ? "var(--blue)" : "var(--border)"}`, background: myPlanValue === v ? "#eff6ff" : "white", cursor: "pointer", textAlign: "left", fontFamily: "inherit" }}>
                  <div style={{ fontWeight: 700, fontSize: 14, color: myPlanValue === v ? "var(--blue)" : "var(--black)" }}>{v.charAt(0).toUpperCase() + v.slice(1)}</div>
                </button>
              ))}
            </div>
