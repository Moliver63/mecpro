...
### Deploy quebrado + cascata de crashes de runtime + correspondencia de segmento por substring (branch fix/deploy-crashes-and-segment-matching, PR pendente de revisao)

Deploy do Render falhou com "Unterminated string literal" em shared/segmentConfig.ts. Investigacao revelou que commits feitos direto no GitHub por Michel (fora do fluxo de PR desta sessao) introduziram varios problemas em cascata, alem do relato original de "acentuacao correta" virando imovel numa confeitaria. Tudo corrigido e validado:

1. **Build quebrado (CORRIGIDO).** 7 ocorrencias de string abrindo com aspas duplas e fechando com crase em shared/segmentConfig.ts (a tentativa de correcao do proprio Michel, commit a32abb2, so consertou 1 das 7). Build (`npm run build`) confirmado passando de verdade apos a correcao.

2. **Chave duplicada silenciosa (CORRIGIDO).** `imagePath` definida duas vezes no mesmo objeto em client/src/pages/GoogleCampaignCreator.tsx (a logica de imagem especifica pra Display, `imgUrl`, nunca era usada — a segunda definicao sempre vencia). Removida a duplicata e a variavel morta.

3. **Crash de runtime na importacao do modulo inteiro (CORRIGIDO).** 6 segmentos novos adicionados a SEGMENT_COPY_RULES em server/ai.ts (veiculos, construcao, educacao, eventos, turismo, pet) usavam `${produto}`/`${empresa}`/`${nichoLabel}` como interpolacao JS real dentro de um objeto de nivel de modulo, onde essas variaveis nao existem — `ReferenceError: produto is not defined` na hora de importar o arquivo, derrubando o SERVIDOR INTEIRO, nao so a geracao de campanhas desses nichos. Corrigido pra placeholder literal (`{produto}`, sem `$`), igual o padrao ja usado pelos segmentos mais antigos. Achadas e corrigidas mais duas ocorrencias do mesmo erro em buildBaseTemplate (uma `nichoLabel` bare inexistente, uma string com aspas duplas que nunca interpolava de verdade).

4. **Crash de runtime em detectSegmentFromNiche pra QUALQUER nicho (CORRIGIDO).** Ao corrigir o item 3, descoberto que os mesmos 6 segmentos nao tinham `nicheKeys` no nivel exigido pela interface SegmentRule (so dentro de `detection.nicheKeys`, campo nao lido por detectSegmentFromNiche) — o loop de deteccao itera todos os segmentos e usa `rules.nicheKeys.some(...)`, entao qualquer chamada de detectSegmentFromNiche (pra QUALQUER nicho, nao so os 6 novos) quebrava ao chegar num desses segmentos. Corrigido adicionando nicheKeys no nivel raiz.

5. **Crash de runtime em getSegmentInstruction pros 6 segmentos novos (CORRIGIDO).** Mesmos 6 segmentos tambem nao tinham os campos obrigatorios de SegmentRule (ctaLeads, ctaSales, copyHook, forbidden, compliance) — `getSegmentInstruction` quebrava com "Cannot read properties of undefined (reading 'slice')" pra qualquer campanha desses nichos. Completados os 5 campos por segmento, com dados extraidos do proprio promptContext ja escrito (CTAs obrigatorios/proibidos ja estavam la como texto).

6. **shared/segmentConfig.ts tinha os MESMOS 6 segmentos duplicados, com formato incompativel (CORRIGIDO por remocao).** Os 6 segmentos tambem foram adicionados em shared/segmentConfig.ts (SEGMENT_CONFIG), num formato completamente diferente do exigido pela interface SegmentDefinition (faltando value/label/icon/desc/ui, copy no formato errado). Confirmado que o unico campo de fato consumido dali (`detection.promptContext`, lido via require() dinamico com fallback silencioso) ja tem equivalente correto em server/ai.ts — removidas as entradas malformadas sem perda funcional.

7. **Correspondencia de nicheKeys por substring solto — MESMA CLASSE DE BUG do "corret"/"correta" relatado por Michel, encontrada de novo (CORRIGIDO na raiz).** Ao investigar o relato original, achado um segundo caso identico: "buffet de casamento" batia com a nicheKey "casa" (de imoveis_venda) por "casamento" conter "casa" como substring. Em vez de corrigir palavra por palavra, criada `matchesNicheKeyword()` (shared/segmentConfig.ts, reaproveitada nos dois sistemas de segmento) exigindo limite de palavra nos dois lados. Isso quebrou correspondencias que dependiam de radical truncado (ex.: "aliment" parando de bater com "alimentacao", "confeit"/"doce" parando de bater com "confeitaria"/"doceria") — corrigido expandindo pras formas completas nos casos que os testes efetivamente pegaram (alimentacao). Nao foi feita uma auditoria manual exaustiva de toda palavra-chave curta de todo segmento (tentativa automatizada gerou falsos positivos demais pra ser confiavel) — a suite de testes ampla (test:fact-guard + test:new-segments, cobrindo exemplos reais de nicho pra cada segmento) e a rede de seguranca contra isso.

8. **Consolidacao da correcao de orcamento-vira-preco (CORRIGIDO).** Um commit direto do Michel (5dd05c1) ja tinha resolvido "orcamento de midia vira preco do produto" com uma funcao PARALELA (`firstMoneyMatchExcludingBudget`) que nao herdava a protecao contra negacao ja existente em firstMatch — reabrindo o bug da campanha #749 pra preco (reproduzido: "nao usar R$18.000 — o correto e R$5.000" voltou a extrair R$18.000). Consolidadas as duas protecoes numa funcao so. No processo, achado e corrigido mais um bug: a janela de contexto de orcamento (100 caracteres) vazava pra frases seguintes, excluindo precos legitimos que so coincidiam de estar perto — restrita a mesma frase.

9. **Preco sem checagem de "nao confirmado" (CORRIGIDO).** Faltava o equivalente pra preco do que ja existia pra endereco (`address_not_confirmed_in_current_briefing`) — sem NENHUM preco confirmado em lugar nenhum, um valor mencionado na copy passava sem checagem nenhuma. Adicionado `price_not_confirmed_in_current_briefing`, mesmo padrao.

Testes: novo server/__tests__/newSegments.test.ts (15 testes — importacao nao quebra, detectSegmentFromNiche nao quebra pra nicho nenhum, getSegmentInstruction nao quebra pra nenhum dos 6 segmentos novos, campos obrigatorios presentes, roteamento correto pros 6 nichos). +4 em campaignFactGuard.test.ts (confeitaria com "correta" nao desvia mais; preco nao confirmado bloqueia; preco confirmado passa; orcamento+negacao juntos). Validado: test:fact-guard 43/43, test:new-segments 15/15, as demais 5 suites sem regressao — 95/95 no total. npm run build confirmado passando de verdade. check:server: 37 erros, todos pre-existentes e ja documentados, zero novo.
