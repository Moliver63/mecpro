...
### Vulnerabilidade critica: valor padrao fixo e publico em endpoints de diagnostico (branch fix/hardcoded-debug-secret, PR pendente de revisao)

Michel pediu auditoria de tudo que um usuario NAO autenticado tem acesso na plataforma. Mapeamento sistematico:

**tRPC — 26 routers, so 4 tem procedimentos publicos (10 no total):** authRouter (register/login/logout — esperado), plansRouter (list/getBySlug — planos de preco, esperado), publicRouter (getLandingMode/getPaymentGateway/getUIConfig — config de landing page, esperado por nome), invitesRouter (getByToken/accept — fluxo de convite antes de ter conta, esperado). Os outros 22 routers (projetos, campanhas, integracoes, financeiro, admin, etc.) exigem protectedProcedure em tudo.

**Rotas Express fora do tRPC:** webhooks (Stripe/Asaas/Resend — publicos por necessidade, validam assinatura internamente), health checks (`/api/health`, publico e de baixo risco), auth inline correta em `/api/campaigns/latest`, `/api/campaigns/:id`, `/api/competitors/status`, `/api/meta/upload-*` (todos exigem JWT valido, sem token retornam null/401 — achado colateral: os dois primeiros sao declarados ANTES de `app.use(cookieParser())`, entao `req.cookies` fica undefined pra eles — nao vaza dado nenhum, so falha fechado pra quem so usa cookie sem tambem mandar o Bearer header; registrado como bug funcional menor, nao de seguranca). `/api/auth/me` exige token, remove passwordHash antes de responder.

**ACHADO CRITICO (CORRIGIDO): `/api/diag/meta` e `/api/health/ai`.** Os dois caiam num valor padrao fixo (`'mecpro-diag-2026'`) quando a variavel de ambiente `DEBUG_TOKEN` nao estava configurada — e esse valor fica visivel pra qualquer pessoa que leia o codigo-fonte deste repositorio publico no GitHub. Sem `DEBUG_TOKEN` configurado no ambiente real, `/api/diag/meta?key=mecpro-diag-2026` expunha, sem login nenhum: email, nome, ID de conta de anuncios, ID do app e um trecho (15 caracteres) do token de acesso de TODOS os usuarios com Meta conectado — e ainda fazia chamadas reais na API do Facebook usando o token de cada cliente (testando permissoes, Ads Library), queimando cota real deles. `/api/health/ai` tinha o mesmo padrao pro modo de diagnostico completo dos provedores de IA. Corrigidos os dois pra falhar fechado: sem `DEBUG_TOKEN` genuinamente configurado no ambiente, o endpoint fica indisponivel (503) — nunca aceita um valor padrao adivinhavel.

**Achado relacionado (CORRIGIDO por consistencia/defesa em profundidade): `/api/admin/credit-balance` e `/api/smoke-test/recharge`.** Os dois caiam pra reaproveitar `JWT_SECRET` como segredo de admin quando `ADMIN_SECRET` nao estava configurado — o mesmo segredo usado pra assinar sessao de todo usuario da plataforma. Nao e um valor PUBLICO como o caso acima (JWT_SECRET e privado), mas reaproveitar um segredo entre dois sistemas diferentes aumenta o raio de dano se ele vazar por qualquer outro caminho (vazamento de JWT_SECRET passaria a tambem dar acesso a credito manual de saldo). Corrigidos pro mesmo padrao: falha fechada (503) sem `ADMIN_SECRET` proprio configurado, sem reaproveitar segredo de outro sistema.

**Frontend:** toda rota privada (30+) esta consistentemente dentro de `<ProtectedRoute>` em App.tsx, incluindo rotas admin com checagem de role. `ProtectedRoute` usa `useAuth()` (que bate em `/api/auth/me`, ja confirmado exigindo token valido) e redireciona pra `/login` sem renderizar nada do conteudo protegido caso nao autenticado — nao e so esconder visualmente, o componente nem monta.

Validado: npm run build confirmado passando (node_modules reinstalado do zero pra evitar falso-positivo de instalacao corrompida), check:server 37/37 (sem erro novo). Correcao e so nos 4 endpoints — nao muda nenhum comportamento pra quem ja usa DEBUG_TOKEN/ADMIN_SECRET configurados corretamente.
