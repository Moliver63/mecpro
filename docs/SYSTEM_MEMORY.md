...
| 🔴 | **NOVO (sessão 28 — 12/08)** — `get_campaign_metrics` (MCP) responde corretamente mas retorna zero registros diários em 100% das campanhas testadas (8 campanhas, múltiplos projetos, inclusive campanha 593 com 20+ dias ativa na Meta e `metaCampaignId` válido). Descarta "sync ainda não rodou" — indica falha sistêmica no pipeline `Meta Insights API → job de sync → banco de snapshots`, não no chart/frontend. Hipótese mais forte: token/permissão (`ads_read`/insights) ou job de sync não está rodando/persistindo em produção. Pode compartilhar causa raiz com o gap de `metric_roas` (linha acima) — resolver este pode destravar os dois. Analisado via Conselho de LLM (framework dos 5 conselheiros); consenso: diagnosticar antes de forçar sync | Dev |
| 🟡 | **NOVO (sessão 28 — 12/08)** — Adicionar ao MCP `get_metrics_sync_status(campaignId)`: retorna timestamp do último sync attempt e erro específico, se houver. Construir e validar PRIMEIRO, contra a campanha 593 real, antes das duas ferramentas abaixo — evita construir "forçar sync" em cima de causa raiz ainda não confirmada | Dev |
| 🟡 | **NOVO (sessão 28 — 12/08)** — Adicionar ao MCP `diagnose_metrics_pipeline()`: diagnóstico de token/permissões/conectividade da conta Meta, escopado por conta (não genérico) — reaproveitar lógica de `list_meta_pages` já existente | Dev |
| 🟡 | **NOVO (sessão 28 — 12/08)** — Adicionar ao MCP `sync_campaign_metrics(campaignId)`: força sincronização manual de uma campanha, com log de retorno explícito (sucesso/quantos registros gravados/erro específico, nunca "ok" genérico). Construir por último, só depois de `get_metrics_sync_status` confirmar a causa. Considerar rate limiting/idempotência antes de expor via chat | Dev |
| 🟢 | **BACKLOG DE PRODUTO (visão Expansionista, sessão 28 — 12/08), não é correção de bug — avaliar depois que o pipeline básico estiver estável:** (1) dashboard de saúde de integração visível pro cliente final ("conta Meta conectada, último sync há X horas") — reforça confiança no momento crítico do primeiro cliente pagante fora do ecossistema; (2) generalizar `diagnose_metrics_pipeline()` em `diagnose_connector_health()` cobrindo qualquer integração externa (publish, upload de criativo, futuras integrações); (3) `sync_campaign_metrics` aceitar `projectId` para sync em lote por cliente, dado o alto volume de campanhas por projeto | Michel |
| 🟢 | **CASO REAL — argumento de peso pro dashboard consolidado (sessão 30 — 12/08).** Auditoria em cliente VIP (Eduardo Oliveira, userId 3, EO Negócios Imobiliários): das 26 campanhas ativas no ad account, só 2 nasceram no MecProAI — as outras 24 foram criadas direto no Gerenciador de Anúncios da Meta (cliente já anunciava lá desde fev/2025, antes de conhecer a ferramenta). Ao analisar performance real via Graph API pra achar a melhor campanha, a de melhor resultado ("Dom Pablo - LOCAÇÃO", R$4,48/contato — a mais eficiente de todo o portfólio, 80 conversas WhatsApp geradas) teve o **anúncio excluído permanentemente na Meta**; a API retornou erro explícito ("não é possível fazer solicitação para objetos excluídos") — a copy vencedora foi perdida pra sempre, só sobrou o agregado numérico do insights. **Se essa campanha tivesse nascido no MecProAI, a copy estaria salva no banco (`campaigns.creatives`/`aiResponse`) independente do que acontecesse na Meta.** Isso reforça o dashboard consolidado (item acima) não só como diferencial, mas como proteção contra perda permanente de dado — vira argumento concreto de venda: "o MecProAI guarda sua copy vencedora mesmo que você apague o anúncio na Meta" | Michel |
