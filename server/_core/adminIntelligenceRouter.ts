...
async function loadCampaignContext(campaignId: number): Promise<{
  ...
  try {
    ...
    niche = normalizeLearningNiche(profile?.niche || aiResp?.niche || campaign.name || "geral");
  } catch {}
...
export const adminIntelligenceRouter = router({
  ...
  getRecommendation: adminProcedure  // recomendação da learning base
    .input(z.object({
      platform:  z.string(),
      objective: z.string(),
