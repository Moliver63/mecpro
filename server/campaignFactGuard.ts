...
// firstMatch — não numa função paralela: uma tentativa anterior criou
// firstMoneyMatchExcludingBudget() separada, que resolvia orçamento mas
// não herdava a proteção contra negação já existente aqui, reabrindo o
// bug da campanha #749 pra preço (reproduzido: "não usar R$18.000 — o
// valor correto é R$5.000" voltou a extrair R$18.000). Uma função só,
// duas proteções.
...
function firstMatch(text: string, patterns: RegExp[], opts?: { preferLongest?: boolean; excludeContext?: RegExp }): string | undefined {
  ...
  for (const pattern of patterns) {
    ...
    while ((m = re.exec(text))) {
      ...
      let contextWindow = text.slice(Math.max(0, m.index - 100), m.index);
